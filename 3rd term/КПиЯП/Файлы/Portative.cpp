...
ifstream& operator>>(ifstream& stream, Portative& c)
{
	stream >> dynamic_cast<Computer&> (c);
	stream >> c.battery >> c.time >> c.screen;

	return stream;
}
ofstream& operator<<(ofstream& stream, Portative& c)
{
	stream << dynamic_cast<Computer&> (c) << ' ' << c.battery << ' ' << c.time << ' ' << c.screen;

	return stream;
}

fstream& operator<<(fstream& out, Portative& c)
{
	out << dynamic_cast<Computer&> (c);

	out.write(reinterpret_cast<char*>(&c.battery), sizeof(int));
	out.write(reinterpret_cast<char*>(&c.time), sizeof(int));
	out.write(reinterpret_cast<char*>(&c.screen), sizeof(double));

	return out;
}
fstream& operator>>(fstream& in, Portative& c)
{
	in >> dynamic_cast<Computer&> (c);

	in.read(reinterpret_cast<char*>(&c.battery), sizeof(int));
	in.read(reinterpret_cast<char*>(&c.time), sizeof(int));
