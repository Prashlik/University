...
ifstream& operator>>(ifstream& stream, SysBlock& c)
{
	stream >> dynamic_cast<Desctop&> (c);
	stream >> c.weight >> c.high >> c.motherboard;

	return stream;
}
ofstream& operator<<(ofstream& stream, SysBlock& c)
{
	stream << dynamic_cast<Desctop&> (c) << ' ' << c.weight << ' ' << c.high << ' ' << c.motherboard;

	return stream;
}

fstream& operator<<(fstream& out, SysBlock& c)
{
	size_t len;

	out << dynamic_cast<Desctop&> (c);

	out.write(reinterpret_cast<char*>(&c.weight), sizeof(double));
	out.write(reinterpret_cast<char*>(&c.high), sizeof(double));

	len = c.motherboard.length();
	out.write(reinterpret_cast<char*>(&c.high), sizeof(double));
	out.write(c.motherboard.data(), len);

	return out;
}
fstream& operator>>(fstream& in, SysBlock& c)
{
	size_t len;
	char* buf;
	...
	in.read(reinterpret_cast<char*>(&c.weight), sizeof(double));
